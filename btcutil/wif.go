...
func DecodeWIF(wif string) (*WIF, error) {
	...
	switch decodedLen {
	case 1 + koblitz.PrivKeyBytesLen + 1 + 4:
		if decoded[33] != compressMagic {
			return nil, ErrMalformedPrivateKey
		}
		compress = true
	case 1 + koblitz.PrivKeyBytesLen + 4:
		compress = false
	default:
		return nil, ErrMalformedPrivateKey
	}
	...
	var tosum []byte
	if compress {
		tosum = decoded[:1+koblitz.PrivKeyBytesLen+1]
	} else {
		tosum = decoded[:1+koblitz.PrivKeyBytesLen]
	}
	...
	netID := decoded[0]
	privKeyBytes := decoded[1 : 1+koblitz.PrivKeyBytesLen]
...
func (w *WIF) String() string {
	...
	// bytes of checksum.
	encodeLen := 1 + koblitz.PrivKeyBytesLen + 4
	if w.CompressPubKey {
		encodeLen++
	}
	...
	a = paddedAppend(koblitz.PrivKeyBytesLen, a, w.PrivKey.D.Bytes())
	if w.CompressPubKey {
...
func (w *WIF) SerializePubKey() []byte {
	pk := (*koblitz.PublicKey)(&w.PrivKey.PublicKey)
	if w.CompressPubKey {
